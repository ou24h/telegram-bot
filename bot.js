...
exec(`./yt-dlp "ytsearch10:${query} site:soundcloud.com" --match-filter "url~='soundcloud.com'" --no-playlist --no-warnings --force-ipv4 --extract-audio --audio-format mp3 --ffmpeg-location "${ffmpegPath}" -o "${filePath}"`, (err, out, errOut) => {
  ...
  if (coverUrl) {
    bot.sendPhoto(chatId, coverUrl, {
      ...
      bot.sendMessage(chatId, `⚠️ تعذر إرسال صورة الكفر:\n${e.message}`);
      bot.sendAudio(chatId, filePath).then(() => {
        fs.unlinkSync(filePath);
      }).catch(e => {
        bot.sendMessage(chatId, `⚠️ تعذر إرسال الملف:\n${e.message}`);
